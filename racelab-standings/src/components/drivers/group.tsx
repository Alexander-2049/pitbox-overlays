...
export const Group = ({
  ...
  const sortedDriversInGroup = useMemo(() => [...drivers].sort((a, b) => a.classPosition - b.classPosition), [drivers])

  const fastestLapInGroup = useMemo(() => {
    return sortedDriversInGroup.reduce(
      (minLap, driver) => (driver.fastestLap !== undefined && driver.fastestLap < minLap ? driver.fastestLap : minLap),
  ...
  const driversToDisplay = useMemo(() => {
    ...
    for (let i = 0; i < sortedDriversInGroup.length && finalDrivers.length < maxDriversToRender; i++) {
    ...
    if (selectedDriver && finalDrivers.length < maxDriversToRender) {
      ...
      while (finalDrivers.length < maxDriversToRender && (left >= 0 || right < sortedDriversInGroup.length)) {
        if (right < sortedDriversInGroup.length && !finalAddedIndices.has(right)) {
  ...
  const groupHeaderColor = groupColor || (drivers[0]?.carClassId ? generateColor(drivers[0].carClassId) : "#DEDEDE")
  ...
  return (
    ...
    >
      ...
      >
        <span>{groupName}</span>
        <span className={`${styles.driverCount} ${themeClass}`}>{drivers.length}</span>
      </div>
      <div
      ...
      {driversToDisplay.map((driver) => (
        <DriverRow
          key={driver.carIdx}
          driver={driver}
          isFastestInGroup={driver.fastestLap === fastestLapInGroup && fastestLapInGroup !== Number.POSITIVE_INFINITY}
